...
public class BundleResourcesImpl implements IBundleResources {
	...
	public InputStream retrieveJar(String symbolicName, String version, String directory) throws TestBundleResourceException {
		...
		// If that didn't work we call listJars to find all the jars in the directory and bundle given
		...
		if (!foundJars.isEmpty()) {
			...
			for (String jar : foundJars) {
				...
				// We don't know exactly what type of jar this is, so we don't know the names of the attributes,
				// but if one has the value of the passed symbolic name it's a fair bet it's the right name
				if (attributes.containsValue(symbolicName)) {

					// If we were passed a version let's inspect all the attributes to see if one of them looks like a version
		...
		throw new TestBundleResourceException("No jar found with symbolicName=" + symbolicName + " and version=" + version);
	}
	...
	/**
	 * Get the integer from a given index in a string array
	 * assume zero if we are out of bounds
	 */
	...
	private boolean isVersionInRange(String version, String range) {
		 ...
		 * (,[\\d+\\.]+)? - as above, prefixed with ',' and optional. should be the upper bound if there is one
		 * ([\\)\\]])$ - reverse of the first group: ']' or ')'
		 */
	...
	@Override
	public InputStream zipDirectoryContents(String resourcesDirectory, Map<String, Object> parameters, String encoding, boolean gzip) throws TestBundleResourceException {
		try {
			...
			if (encoding == null) {
				...
				for (String path : contents) {
					...
					String zipEntryName = path.replaceFirst("\\/*" + RESOURCES_DIRECTORY + "\\/*" + resourcesDirectory + "\\/*", "");
	...
	@Override
	public InputStream retrieveSkeletonFile(String path,
			Map<String, Object> parameters) throws TestBundleResourceException {
