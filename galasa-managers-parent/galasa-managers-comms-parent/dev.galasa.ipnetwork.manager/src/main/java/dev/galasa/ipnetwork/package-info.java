 ...
 *  your tests. The CPS defines object properties, so by using this store to access 
 *  hostnames and port number means that the tester does not need to know where the 
 *  application to which they are connecting is. The system administrator can set different 
 *  properties in the CPS to allow a test to run against multiple environments without 
