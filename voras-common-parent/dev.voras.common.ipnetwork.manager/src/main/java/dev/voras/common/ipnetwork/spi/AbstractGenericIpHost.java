...
public abstract class AbstractGenericIpHost implements IIpHostSpi {
	...
	public AbstractGenericIpHost(IConfigurationPropertyStoreService cps,
			IDynamicStatusStoreService dss,
			ICredentialsService creds,
			String prefix, 
			String hostid) throws IpNetworkManagerException {
		this.cps    = cps;
		this.dss    = dss;
		this.creds  = creds;
		this.prefix = prefix;
		...
		try {
			...
			if (hostnameIPV4 == null && hostnameIPV6 == null) {
				throw new IpNetworkManagerException("Unable to locate a hostname for hostid " + hostid);  //*** TODO give example property ids
			}
	...
	public boolean isFtpPortTls() throws IpNetworkManagerException {
		try {
			return Boolean.parseBoolean(AbstractManager.nulled(this.cps.getProperty(this.prefix, "ftp.tls", this.hostid)));
		} catch(Exception e) {
	...
	public @NotNull ICredentials getDefaultCredentials() throws IpNetworkManagerException {
		try {
			...
			if (creds == null) {
				throw new IpNetworkManagerException("Unable to obtain default credentials, the credentials id for host " + temp + " is missing");
			}
			return creds;
		} catch(IpNetworkManagerException e) {
			throw e;
