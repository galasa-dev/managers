...
public class PlusPlusSkeletonProcessor implements ISkeletonProcessor {
	...
	private final Delimiter[] delimiters = { 
			new Delimiter("++", true) // RTS style
//			new Delimiter("&", "", false) // SEM style
	};

	// Define all loop formats handled
	private final LoopForm[] loopForms = { 
			new LoopForm("<for " + LoopForm.MEMBER 
					+ " in ++" + LoopForm.COLLECTION + "++>", "</for>") 
	};

	private final LoopLimiter[] loopLimiters = { 
			new LoopLimiter("<x" + LoopLimiter.LIMIT + ">", "</x>") 
	};

	private final Anchor[] anchors = { 
			new Anchor("<a" + Anchor.COLUMN + ">") 
	};
	...
	@Override
	public InputStream processSkeleton(InputStream skeleton,
			Map<String, Object> parameters) throws SkeletonProcessorException {
		...
		for (int i = 0; i < 2; i++) {
			...
			for (Entry<String, Object> substitution : parameters.entrySet()) {
				...
				if (substitution.getValue() instanceof String) {
					skeletonString = directSubstitute(skeletonString,
							substitution.getKey(),
					...
					skeletonString = loopSubstitute(skeletonString,
							substitution.getKey(),
	 ...
	 * @return
	 */
	private String directSubstitute(String skeletonContent, String key,
			String value) {
	 ...
	 */
	private String loopSubstitute(String skeletonContent, String key,
			Collection<?> collection) throws SkeletonProcessorException {

		for (LoopForm loopForm : loopForms) {
			skeletonContent = loopForm.substitute(skeletonContent, key,
					collection);
		}
	 ...
	 * @throws SkeletonProcessorException
	 */
	private String reAlign(String skeletonContent)
			throws SkeletonProcessorException {
	...
	public String purgeUnusedMarkers(String skeletonContent) {
		...
		for (LoopForm loopForm : loopForms) {
			skeletonContent = skeletonContent.replaceAll(
					loopForm.getGeneralRegexPattern(), "");
			Pattern p = Pattern.compile(loopForm.getGeneralRegexPattern(),
	...
	private class Delimiter {
		...
		/**
		 * Construct a Delimiter which has the same start and end delimiters
		 * e.g. ++var++
		 * 
		 * @param delimiter
		 */
		...
		/**
		 * Construct a Delimiter with different start and end delimiters e.g.
		 * ${var}
		 * 
		 * @param start
		 * @param end
		...
		public String substitute(String input, String key, String value) {
			...
			Pattern p = Pattern.compile(getRegexPattern(key),
			...
			while (m.find()) {
				output = output.substring(0, m.start())
						+ value
						+ output.substring((end.length() > 0) ? m.end() : m
								.end() - 1);
				m = p.matcher(output);
			}
			return output;
	...
	/**
	 * Describes a known way of describing a substitution for each member of
	 * some collection
	 * 
	 * @author root
	 * 
	...
	private class LoopForm {
		...
		public String getGeneralRegexPattern() {
			...
			sb.append("\\Q");
			sb.append(loopOpen.replaceFirst(MEMBER, "\\\\E\\\\w+?\\\\Q")
					.replaceAll(COLLECTION, "\\\\E\\\\w+?\\\\Q"));
			sb.append("\\E\\r?\\n?.+?\\Q");
			sb.append(loopClose.replaceAll(MEMBER, "\\\\E\\\\w+?\\\\Q")
					.replaceAll(COLLECTION, "\\\\E\\\\w+?\\\\Q"));
			sb.append("\\E\\r?\\n?");
		...
		public String substitute(String output, String key, Collection<?> values)
				throws SkeletonProcessorException {

			Pattern p = Pattern.compile(getRegexPattern(key), Pattern.DOTALL
					| Pattern.CASE_INSENSITIVE);
			...
			while (m.find()) {
				...
				if (m.groupCount() != 2) {
					throw new SkeletonProcessorException(
							"The pattern matcher for this loop has returned an "
				...
				for (LoopLimiter limiter : loopLimiters) {
					Pattern repeatLimiter = Pattern.compile(limiter
							.getRegexPattern());
					...
					if (limitMatcher.find()) {
						...
						phrase = phrase.substring(0, limitMatcher.start())
								+ limitee
				...
				for (Object sub : values) {
					if (!(sub instanceof String)) {
						throw new SkeletonProcessorException(
								"The collection given to substitute for '"
										+ key
				...
				output = output.substring(0, m.start()) + sb.toString()
						+ output.substring(m.end());
	...
	private class Anchor {
		...
		public String reAlign(String output) throws SkeletonProcessorException {
			...
			Pattern p = Pattern.compile(getRegexPattern(),
			...
			while (m.find()) {
				...
				if (m.groupCount() != 3) {
					throw new SkeletonProcessorException(
							"The pattern matcher for this anchor has returned an "
									+ "unexpected number of groups.");
				}
				...
				StringBuilder sb = new StringBuilder();
				sb.append(String.format("%-" + (columnNumber - 1) + "s",
						staticPhrase));
